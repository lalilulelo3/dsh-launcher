...
> ⚠️ **请使用 v1.2.1 或更新版本。**
> **v1.1.0 / v1.2.0 有一个致命回归**：只要**存在被禁用的插件**（或使用「安全模式启动」），
> 启动 Harness 会直接失败，报错形如
> `failed to read overlay D:\"C:\Users\<你>\.dsh\launcher\disabled.patch.yml"`。
> 原因是传给 dsh 的参数里，`--patch` 的文件路径被多加了一对双引号，路径不再被识别为绝对路径。
...
- **为什么不经 shell 时参数绝不能加引号**：v1.1.0 起启动方式改成「直接起 `node` + `lib/bin.js`」，
  参数以**列表**形式交给 CreateProcess —— 但 `--patch` 的路径沿用了为 shell 准备的引号化。
  不经 shell 时引号**不会被剥掉**，于是成了参数的一部分：Node 判定它不是绝对路径，按当前
  目录解析、前缀上当前盘符，变成 `D:\"C:\…\disabled.patch.yml"` 而读不到文件，
  **禁用过插件就必然启动失败**（而且只在传 `--patch` 时触发，开发机上一路没暴露，
  直到用户在家里那台 exe 位于 D: 盘的机器上踩到）。
  现在 `dsh_launch_spec()` 一律返回**不带引号**的参数，只有需要拼成一条 shell 字符串时
  才由调用方逐个引号化；`tests/test_launch_args.py` 把这条规则钉死了
  （它会抓住真正传给 `Popen` 的参数，并用 Node 复现"引号变路径一部分"的机制）。
